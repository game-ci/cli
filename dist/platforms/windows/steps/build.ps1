...
#
if ($Env:GAME_CI_UNITY_SETTINGS)
{
    if (Test-Path -Path "$(Join-Path $Env:ACTION_FOLDER 'settings-applier\Assets\Editor')")
    {
        Write-Output "Applying GameCI Unity settings spec - copying settings applier into the project."

        if (-Not (Test-Path -Path $Env:UNITY_PROJECT_PATH\Assets\Editor))
        {
            New-Item -ItemType Directory -Force -Path $Env:UNITY_PROJECT_PATH\Assets\Editor | Out-Null
        }

        Copy-Item -Path "$(Join-Path $Env:ACTION_FOLDER 'settings-applier\Assets\Editor')\*" -Destination $Env:UNITY_PROJECT_PATH\Assets\Editor -Recurse -Force
    }
    else
    {
        Write-Output "##[warning] GAME_CI_UNITY_SETTINGS is set but the settings applier was not found - settings will not be applied."
    }
}
