...
if [ -n "$PRIVATE_REGISTRY_TOKEN" ]; then
  ...
  alwaysAuth = true
EOF
  # Some npm repositories (Azure Devops) require a user to be configured
...
if [ "$PACKAGE_MODE" = "true" ]; then
  ...
  if [ ! -f "$PACKAGE_MANIFEST_PATH" ]; then
      echo "Packages/manifest.json was not created properly. This indicates a problem with the CLI, not with your package. Logging directories and aborting..."
...
#
COVERAGE_FLAGS=()
if [ -z "$COVERAGE_ENABLED" ] || [ "$COVERAGE_ENABLED" = "true" ]; then
  COVERAGE_FLAGS=(-coverageResultsPath "$FULL_COVERAGE_RESULTS_PATH" -enableCodeCoverage -debugCodeOptimization -coverageOptions "$COVERAGE_OPTIONS")
else
  echo "Code coverage disabled (coverageEnabled=false)."
fi
...
for platform in ${TEST_PLATFORMS//;/ }; do
  if [[ "$platform" == "standalone" ]]; then
    ...
    # Copy the scripts
    cp -R "${TEST_RUNNER_ACTION_DIR:-/UnityTestRunnerAction}/Assets/Editor/" "$UNITY_PROJECT_PATH/Assets/Editor/"
    cp -R "${TEST_RUNNER_ACTION_DIR:-/UnityTestRunnerAction}/Assets/Player/" "$UNITY_PROJECT_PATH/Assets/Player/"
...
# TEST_RUNNER_EXIT_CODE stays 0 if every platform above passed (loop only
# ever raises it, never resets it back down).
if [ -z "$TEST_RUNNER_EXIT_CODE" ]; then
...
elif [ -z "$COVERAGE_ENABLED" ] || [ "$COVERAGE_ENABLED" = "true" ]; then
