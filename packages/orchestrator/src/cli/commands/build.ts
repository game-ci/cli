  ...
  builder: (yargs) => {
    return (
      yargs
        ...
        // Note: engine-specific licensing flags (--skip-activation,
        // --unity-licensing-server, --unity-licensing-toolset, --unity-serial)
        // are not exposed here. Pass them via environment variables (e.g.
        // UNITY_SERIAL, UNITY_LICENSING_SERVER) which are forwarded to the
        // build container automatically, or via the engine-specific host
        // entry point (unity-builder action / @game-ci/cli). See
        // https://github.com/game-ci/orchestrator/issues/25
