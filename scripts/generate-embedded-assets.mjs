...
// Hashes the uncompressed content, deliberately. gzip output is not
// byte-reproducible across zlib versions - the same input compressed under
// Node 20 and Node 24 differs - so hashing the compressed bytes made the
// cache key depend on the toolchain that built it, and made the drift check
// fail purely from a Node version difference. The raw content is what the
// cache is actually addressing.
...
// --check verifies the committed module matches dist/ without requiring the
// regenerated file to be byte-identical, since the compressed payload varies
// by zlib version. Compares what actually matters: the content hash and the
// file index.
if (process.argv.includes('--check')) {
  const committed = fs.readFileSync(outFile, 'utf8');
  const committedHash = committed.match(/EMBEDDED_ASSETS_HASH = '([^']+)'/)?.[1];
  const committedIndex = committed.match(/EMBEDDED_ASSETS_INDEX[^=]*= (\[.*?\]);/s)?.[1];

  if (committedHash !== hash || committedIndex !== JSON.stringify(index)) {
    console.error(
      `src/generated/embedded-assets.ts is out of date.
` +
        `  committed content hash: ${committedHash}
` +
        `  dist/ content hash:     ${hash}
` +
        `Run 'bun run build:assets' and commit the result.`,
    );
    process.exit(1);
  }

  console.log(`Embedded assets are up to date (content hash ${hash}).`);
  process.exit(0);
}
